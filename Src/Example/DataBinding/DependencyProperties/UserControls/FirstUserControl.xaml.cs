...
namespace Example.WPF.DataBinding.DependencyProperties.UserControls
{
    ...
    public partial class FirstUserControl : UserControl
    {
        ...
        /// <summary>
        /// 建立一屬性用於存取相依屬性 (Dependency Property) 的值
        /// </summary>
        /// <remarks>
        /// <para>此屬性只是相依屬性的包裝(Wrapper)</para>
        /// </remarks>
        public double FirstUserControlValue
        {
            ...
            {
                /*
                 * 透過 GetValue 方法取得相依屬性 (Dependency Property) 的值
                 ...
                 */
                return (double)GetValue(FirstUserControlValueProperty); 
            }
            set 
            {
                /*
                 * 透過 SetValue 方法設定相依屬性 (Dependency Property) 的值
                 * 資料綁定同步相依屬性時，系統會直接呼叫 DependencyObject.GetValue 和 DependencyObject.SetValue 來操作屬性，
                 * 並不會透過 FirstUserControlValue，因此不應該在 FirstUserControlValue 的 setter 中加入其他的指令
                 */
                SetValue(FirstUserControlValueProperty, value); 
            }
        }
    }
}
